...
{
    int n = g.numVertices();
    // colour[v] = assigned colour, -1 if uncoloured
    std::vector<int> colour(n, -1);
    int used = 0;
    auto adjC = g.complement(); // bool** of complement

    for (int v : order)
    {
        if (!active[v])
            continue;
        // mark forbidden colours by neighbours in the complement
        std::vector<char> forbidden(used, 0);
        for (int u = 0; u < n; ++u)
        {
            if (active[u] && colour[u] != -1 && adjC[v][u])
            {
                forbidden[colour[u]] = 1;
            }
        }
        // pick smallest free colour
        int c = 0;
                       ...
                       std::vector<int> &best)
{
    ...
    // first the trivial bound
    if ((int)current.size() + rem <= (int)best.size())
        return;

    // now the colouring‐based bound on the complement
