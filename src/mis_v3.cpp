...
// ALGORITHM (START)
/**
 *  Algo v2 + pruning of partial solutions that can't become optimal.
 */

void branchAndBoundRec(const Graph &g, int idx, std::vector<int> &current, std::vector<int> &best, int remainingVertices)
{
    int n = g.numVertices();
    ...
    // check if current solution has the potential to be better than the current best solution
    if (current.size() + (n - idx) <= best.size())
    {
        return;
    }
    ...
    if (canInclude)
    {
        ...
        branchAndBoundRec(g, idx + 1, current, best, remainingVertices - 1);
        current.pop_back();
    }
...
int branchAndBound(const Graph &g, std::vector<int> &independentSet)
{
    ...
    branchAndBoundRec(g, 0, current, independentSet, g.numVertices());
